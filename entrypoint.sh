...
echo "c.LocalAuthenticator.add_user_cmd = ['/adduser.sh']" >> /srv/jupyterhub/jupyterhub_config.py
...
useradd -d $NB_VOLUME/$NB_USER -u $NB_UID -g $NB_GID -p `echo "$NB_PASSWORD" | mkpasswd -s -m sha-512` $NB_USER || exit 1
